...
var log = logging.MustGetLogger("goplays")
...
func Init() {
	var logLev string
	flag.StringVar(&logLev, "stderrthreshold", "", "logs at or above this threshold go to stderr")
	flag.Parse()

	//set log level
	lev, err := logging.LogLevel(logLev)
	if err != nil {
		panic(err)
	}
